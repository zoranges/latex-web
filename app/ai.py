...
_CUMCM_RULES = r"""这是全国大学生数学建模竞赛（高教社杯）论文的排版任务。以下是 2026 年全国组委会规范中的硬性要求，未列出的字号、字体、行距和颜色不要擅自当作全国统一要求：

【硬性格式要求】
1. A4 纸，上下左右页边距至少 2.5cm；不要添加目录。
...
【电子版与 AI 排版】
13. 本工具默认生成电子版论文内容：不主动新增承诺书和编号专用页。若输入文件已有这两页，不要擅自改写其内容，应在 summary 中提醒用户提交电子版时移除，并另附当届官方专用页用于纸质版。
